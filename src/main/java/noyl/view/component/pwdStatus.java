...
import java.awt.*;
import noyl.util.PasswordValidation;

public class pwdStatus extends JPanel {
    ...
    private void checkPassword(String password) {
        this.type = password.isEmpty() ? 0 :  PasswordValidation.isPwdValid(password);
        if (type == 0) {
            label.setText("none");
            label.setVisible(false);
        } else {
    ...
    public void initPasswordField(JTextField txt) {
        if (documentListener == null) {
            documentListener = new DocumentListener() {
                @Override
                public void insertUpdate(DocumentEvent e) {
                    checkPassword(String.valueOf(txt.getText()));
                }
